...
export function useAiBootstrap(): {
  ...
  // Inline id check (not isClaudeCodeModel) so this eagerly-loaded startup hook
  // never pulls claudeCli.ts → the `ai` SDK into the main bundle. See
  // src/app/eager-budget.test.ts.
  const hasComposer =
    hasAnyKey(apiKeys) ||
    hasLocalModel ||
    selectedModelId === "claude-code-sonnet";
